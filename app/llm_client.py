...
def generate_quiz_batch(
    ...
    system_prompt = (
        ...
        "based strictly on the 24 slide chunks provided. For each slide chunk, generate exactly one question "
        ...
        "CRITICAL RULES:\n"
        "1. All questions, options, code comments, and correct answers must be written in English. If the source slide chunk is in Romanian, translate the concepts and text to English.\n"
        "2. The provided few-shot examples might be in Romanian, but you MUST translate the patterns and output only in English.\n"
        "3. Avoid meta-questions or administrative questions about course syllabus, schedule, agenda, slide/lecture numbers, or what is explicitly listed/mentioned on a slide (e.g., 'According to slide X, which topic is not discussed?'). Focus strictly on technical Prolog concepts, logical reasoning, search algorithms, database manipulation, grammar (DCGs), cuts, backtracking, and standard Prolog structures.\n"
        "4. Multiple choice options must be clear, distinct, and conceptually test Prolog knowledge.\n"
        "5. Difficulty: similar to university exams. Be rigorous and clear.\n\n"
        "6. Questions must be self-contained. Never refer to an unseen image, diagram, figure, or graph.\n"
        "7. If a question requires a graph, include a visual object of type graph with explicit nodes and edges. "
        "If you cannot reconstruct the required graph from the slide text, do not generate a graph-dependent question.\n\n"
